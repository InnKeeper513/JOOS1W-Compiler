...
class A4Tests extends FunSuite {
  ...
  val stdlib: Seq[File] = allFilesInDir("resources/testcases/stdlib/4.0")
  ...
  val root = new File("resources/testcases/a4")
  ...
  test("single file test") {
    val file = new File(root.getAbsolutePath + "/Je_7_DefiniteAssignment_2LazyOr_Assignment.java")
    assertRejectBecauseOf[Throwable](file.getName, Seq(file))
  }
