...
import Asts.CompilationUnit
import Scanner.tokenizeJoos1w
import Weeding.weedJoos1w
import Asts.treeToAst
...
object Parser {
  ...
  def compileFile(filename: String): CompilationUnit = {
    ...
    val classnames = collect(root, "InterfaceDeclaration") ++ collect(root, "ClassDeclaration")
    ...
    classnames.head.children.foreach(name => {
      if (name.lhs.kind == "IDENTIFIER" && name.lhs.lexeme != word.substring(0, word.length - 5)) {
        throw new Exception("File Name Must Match With Class or Interface Name.")
      }
    })
    weedJoos1w(root)
