...
import A2.Asts.QualifiedName
import A2.Environments.RootEnvironment
...
object Utils {
  ...
  def allFilesInDir(dir: String): Seq[File] = {
    ...
    var files = recur(d)
    def sortByMain(f1: File, f2: File) : Boolean = {
      if (f1.getName == "Main.java") {
        return true
      } else if (f2.getName == "Main.java") {
        return false
      }
      return f1.getName < f2.getName
    }
    files.sortWith(sortByMain)
  }
