    ...
    ast match {
      case b: BinaryExpression =>
        ...
        if (b.exp1.constEval.nonEmpty && b.exp2.constEval.nonEmpty) {
          ...
          // all right this is ugly... but this is the best design I can come up with right now...
      ...
      case c: CastExpression =>
        ...
        if (c.exp.constEval.nonEmpty) {
          // as long as it passed all the tests...
      ...
      case l: LiteralExpression =>
        l.constEval = l.literal match {
          case i: IntegerLiteral => Some(ConstInt(i.getLexeme))
          case c: CharLiteral => Some(ConstChar(c.getLexeme))
          case s: StringLiteral => Some(ConstString(s.getLexeme))
          case b: BooleanLiteral => Some(ConstBoolean(b.getLexeme))
          case n: NullLiteral => None
        }
      ...
      ast match {
        ...
        case importDecl: ImportDecl =>
          if (importDecl.onDemand) {
            ...
            // no single-type-import declarations may resolve to types
        ...
        case v: VariableExpression =>
          v.variableLocation match {
            case VariableLocation.LocalVariable =>
              if (isAssignment || v.accessName.names.nonEmpty) {
