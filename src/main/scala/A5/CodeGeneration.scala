...
import A4.StaticAnalysis.joos1wA4
import A5.Label._
...
object CodeGeneration {
  ...
  def emitExpression(exp: Expression): Unit = {
    exp match {
      ...
      case c: CastExpression =>
        emit(s"mov ${Reg.ecx}, 0")
        emitCastExpression(c.exp, c.cType, true)
        emit(s"cmp ${Reg.ecx}, 1")
  ...
  def emitInstanceOfExpression(expression: Asts.InstanceOfExpression): Unit ={
    ...
    emit(s"mov ${Reg.ecx}, 0")
    emitCastExpression(expression.exp, expression.iType, false)
    emit(s"mov ${Reg.eax}, ${Reg.ecx}")
    emit(s"xor ${Reg.eax}, 1")
    emit("", "End of InstanceOfExpression")
  }

  def emitCastExpression(c: Expression, t: Type, isCast: Boolean): Unit = {
    emit("", "Begin of CastExpression")
    emitExpression(c)
    emit(s"push ${Reg.eax}")
    val jumpLabel = createLabel()
    // Null Check on the expression
    if(!isCast) {
      emit(s"cmp ${Reg.eax}, 0", s"null check on register ${Reg.eax}")
      emit(s"sete ${Reg.cl}")
      emit(s"movzx ${Reg.ecx}, ${Reg.cl}")
      emit(s"je $jumpLabel")
    }
    if(t.isInstanceOf[NullType]){
      emit(s"mov ${Reg.ecx}, 1")
      emit(s"je $jumpLabel")
    }
    // target, src
    (t, c.astType.get) match {
      ...
      case (_: InterfaceType, _: ArrayType) => throw CodeGenerationException("Bad Cast")
      case (target: ArrayType, source: InterfaceType) => throw CodeGenerationException(s"Bad Cast in $source to target type $target")
        // a class type casted to an array type
      case (target: ArrayType, source : ClassType) => ArrayCastClass(target.aType, source.name.toString)
      case (target: ArrayType, _: ArrayType) => ArrayCastArray(target.aType)

      case (_, _) =>
        // assume all the other cases are invalid
        CodeGenerationException("Bad Cast")
    }
    def ArrayCastClass(aType: Type, source: String): Unit = {
      val jumpLabel = createLabel()
      val aTypeName = aType match{
        case interfaceType: InterfaceType => interfaceType.name.toString
        case classType: ClassType => classType.name.toString
        case _ =>
      }
      if(aTypeName == source && source == "java.lang.Object" && isCast){
        /*
        Special case
        Object o = new Object();
        o = (Object[])o;
         ...
         */
        return
      } else if(source == "java.lang.Object"){
        emit(s"push ${Reg.eax}")
        emit(s"mov ${Reg.ebx}, 0")
        emit(s"cmp ${Reg.eax}, ${Reg.ebx}")
        emit(s"je $jumpLabel")
        ArrayCastArray(aType)
        emitLabel(jumpLabel)
        emit(s"pop ${Reg.eax}")
        return
      }
    ...
    def checkArraySubType(aType: Type): Unit ={
      ...
      //emit(s"je __exception")
        ...
        val subtypeTableIndex = classAndInterfaceNames.indexOf(aTypeName) + 1
    ...
    def ReferenceCastReference(targetName: QualifiedName): Unit = {
      ...
      // Check the offset of target in the classAndInterfaceName Table
      val subtypeTableIndex = classAndInterfaceNames.indexOf(targetName) + 1
      ...
      // Extract the value of the target from the subtype table
      // Value 1 should be able to cast, Value 0 should not be able to cast
      emit(s"mov ${Reg.eax}, [${Reg.eax} + $subtypeTableIndex * 4]", s"Move extract subtype information of $targetName with offset $subtypeTableIndex")
...
//  }
  ...
  def emitClassInstanceCreationExpression(expression: ClassInstanceCreationExpression): Unit = {
    emit("", s"Begin of ClassInstanceCreationExpression $expression")
    // create object
    ...
    val arguments: Seq[Type] = expression.args.map(_.astType.get)
    ...
    emit(s"lea ${Reg.esi}, [4 * ${expression.args.size} + ${Reg.esp}]", "save parameter ptr")
    val classInstanceType = expression.cType.name.getQualifiedName
    emit(s"call ${createConstructorLabel(classInstanceType.toString, arguments)}", "call constructor")
    ...
    emit("", s"End of ClassInstanceCreationExpression $expression")
  }
  ...
  def emitVariableExpression(expression: VariableExpression): Unit = {
    emit("", s"Begin of VariableExpression $expression")
    expression.variableLocation match {
      case VariableLocation.LocalVariable =>
        ...
        emit(s"mov ${Reg.eax}, [${Reg.ebp} - ${4 * offset} - 4]", s"move local variable ${expression.name.getFirstSimpleName} value with offset ${offset} to ${Reg.eax}")
        ...
        for (fieldStr <- expression.accessName.names) {
          currentType match {
            case arrayType: ArrayType =>
              ...
              emit(s"mov ${Reg.eax}, [${Reg.eax} + $arrayLengthOffset * 4]", "access array length")
      ...
      case VariableLocation.LocalField =>
        ...
        val isStatic = classScope.gatherFields(expression.name.getFirstSimpleName)._2.contains(Modifier.STATIC)
        ...
        if (isStatic) {
          ...
          emit(s"mov ${Reg.eax}, [${Reg.eax} + ${fieldOffset * 4}]", s"access field ${expression.name.getFirstSimpleName} with offset $fieldOffset")
        }
        ...
        for (fieldStr <- expression.accessName.names) {
          fieldInfo._1 match {
            case arrayType: ArrayType =>
              ...
              // this must be the last fieldStr and the field must be length
              // which is checked by disambiguation
              emit(s"mov ${Reg.eax}, [${Reg.eax} + 4 * $arrayLengthOffset]", "access array length")
            case classType: ClassType =>
          ...
          currentType match {
            ...
            case classType: ClassType =>
              ...
              if (index == 0) {
                ...
                emit(s"mov ${Reg.eax}, $staticFieldLabel", "get static label address")
                emit(s"mov ${Reg.eax}, [${Reg.eax}]", "read static field")
              } else {
                val fieldInfo = classScope.gatherNonStaticFields(SimpleName(fieldStr))
                currentType = fieldInfo._1
                val fieldOffset = 3 + fieldInfo._3
  ...
  def emitStringArrayCreationExpression(stringLiteral: String): Unit ={
    emit("", s"Begin of ArrayCreationExpression")
    // first allocate the memory for char array
    emit("", "Begin of create char array for java.lang.String")
    emit(s"mov ${Reg.eax}, ${stringLiteral.length() * 4}")
    emit(s"add ${Reg.eax}, ${arrayStartOffset * 4}")
    emit("call __malloc")
    emit(s"mov ${Reg.ebx}, $createPrimitiveArrayLabel")
    emit(s"mov [${Reg.eax}], ${Reg.ebx}")
    emit(s"mov ${Reg.ebx}, 4")
    emit(s"mov [${Reg.eax} + ${arrayTagVTableOffset * 4}], ${Reg.ebx}")
    emit(s"mov ${Reg.ebx}, 0x${stringLiteral.length.toHexString}", s"move string literal length ${stringLiteral.length} into ebx")
    emit(s"mov [${Reg.eax} + ${arrayLengthOffset * 4}], ${Reg.ebx}")
    stringLiteral.zipWithIndex.foreach {
      character =>
        emit(s"mov ${Reg.ebx}, 0x${character._1.toInt.toHexString}", s"mov character into ebx")
        emit(s"mov [${Reg.eax} + ${arrayStartOffset * 4 + character._2 * 4}], ${Reg.ebx}")
        // emit(s"mov dword [${Reg.eax} + ${arrayStartOffset * 4 + character._2 * 4}], 0x${character._1.toInt.toHexString}", s"mov character '$character' into eax")
    }
    emit(s"push ${Reg.eax}", "save the character array address to the stack")
    emit("", "End of create char array for java.lang.String")

    // allocate memory for java.lang.String
    emit("", "Begin of create java.lang.String")
    emit(s"lea ${Reg.eax}, [4 + $arrayStartOffset * 4]")
    ...
    emit(s"pop ${Reg.ebx}", "pop the char array address into ebx")
    emit(s"mov [${Reg.eax} + 4 * $arrayStartOffset], ${Reg.ebx}")
    emit("", "End of create java.lang.String")
    emit("", s"End of ArrayCreationExpression")
  }
  ...
  def emitLiteralExpression(expression: LiteralExpression): Unit = {
    ...
    expression.literal match {
      case i: IntegerLiteral =>
        emit(s"mov ${Reg.eax}, ${i.getLexeme}", s"integer literal with value ${i.getLexeme}")
      case c: CharLiteral =>
        emit(s"mov ${Reg.eax}, 0x${c.getLexeme.toHexString}", s"character literal with value ${c.getLexeme}")
      ...
      case n: NullLiteral =>
        emit(s"mov ${Reg.eax}, 0", "null literal")
      case s: StringLiteral =>
        emitStringArrayCreationExpression(s.getLexeme)
    }
  ...
  def checkArrayAccessType(arrayAccessExpression: ArrayAccessExpression, rhsExpression: Expression): Unit ={
    // Index 0 General array type
    // Index 1 Primitive or Reference Type
    // eax is now the address of the array
    emit("", "Begin Array Access Check")
    val jumpLabel = createLabel()

    emit("", "Begin of ArrayAccessExpressionAddress")
    emitExpression(arrayAccessExpression.exp1)
    emit(s"push ${Reg.eax}", "push the result of ArrayAccessExpression.exp1 onto stack")

    emitExpression(arrayAccessExpression.exp3)
    emit(s"pop ${Reg.ebx}", s"pop the result of ArrayAccessExpression.exp1 to ${Reg.ebx}")
    ...
    emit(s"mov ${Reg.eax}, ${Reg.ebx}")
    emit(s"push ${Reg.ebx}")
    if(rhsExpression.astType.get.isInstanceOf[PrimitiveType]){
      emit(s"pop ${Reg.ebx}")
      // Primitive Index
      // BOOLEAN: 1
      // BYTE: 2
      ...
      // CHAR: 4
      // INT: 5
      //emit(s"mov ${Reg.ebx}, [${Reg.ebx}]", "move inside the array")
      emit(s"mov ${Reg.ecx}, 5")
      emit(s"cmp [${Reg.ebx}+4], ${Reg.ecx}")
      emit(s"jg __exception")
      emitExpression(rhsExpression)
    } else {
      // Check reference type casting
      // Eax now contains the address to the RHS type
      emitExpression(rhsExpression)
      // Eax now holds the address of the object
      emit(s"pop ${Reg.ebx}", "Get the address of the array")

      // Get array's subtype table index
      emit(s"mov ${Reg.ecx}, [${Reg.ebx} + 4]", "Move into array's vtable")
      emit(s"mov ${Reg.ecx}, [${Reg.ecx} + 4]", "Move into array's vtable's subtype table")
      emit(s"mov ${Reg.ecx}, [${Reg.ecx}]", "Get the subtype index value")

      emit(s"push ${Reg.eax}")
      // eax is the address of the source expression
      // Move into vtable
      emit(s"mov ${Reg.eax}, [${Reg.eax}]", "Move into object's vtable")
      // Navigate to subtype table, which is index 1 of the vtable
      emit(s"mov ${Reg.eax}, [${Reg.eax} + 4]", "Move into subtype table")
      // Extract the value of the target from the subtype table
      // Value 1 should be able to cast, Value 0 should not be able to cast
      emit(s"mov ${Reg.eax}, [${Reg.eax} + ${Reg.ecx} * 4 + 4]")
      emit(s"cmp ${Reg.eax}, 0", "Throw exception if not able to cast")
      emit(s"je __exception")
      emit(s"pop ${Reg.eax}")

      // After a successful access, we will change the array type
      emit(s"push ${Reg.eax}")
      emit(s"mov ${Reg.eax}, [${Reg.eax}]")
      // We have the RHS's Vtable
      // Now assign it to the LHS array (replace the array's vtable with this new vtable)
      emit(s"add ${Reg.ebx}, 4")
      emit(s"mov [${Reg.ebx}], ${Reg.eax}")

      emit(s"pop ${Reg.eax}")
    }
    emit("", "End Array Access Check")
  }
  ...
  def emitAssignment(assignment: Assignment): Unit = {
    ...
    assignment.lhs match{
      case arrayAccessExpression: ArrayAccessExpression =>
        checkArrayAccessType(arrayAccessExpression, assignment.rhs)
      case _ =>
  ...
  def emitFieldAccessExpressionAddress(f: FieldAccessExpression): Unit = {
    ...
    def emitNonStaticFieldAccessExpressionAddress(f: FieldAccessExpression): Unit = {
      ...
      nullCheck(Reg.eax)
      val statementScope = f.environment.get.asInstanceOf[StatementScope]
      val typeScope = statementScope.getScopeForType(f.exp.astType.get.asInstanceOf[ClassType].name.getQualifiedName)
      val offset: Int = typeScope.gatherNonStaticFields(f.name)._3
      emit(s"add ${Reg.eax}, ${offset * 4 + arrayStartOffset * 4}", s"adding the offset $offset to the base address of the class for nonstatic field ${f.name}")
    ...
    if (f.exp.astType.get.isInstanceOf[ArrayType] && f.name == SimpleName("length")) {
      ...
      f.exp.astType.get match {
        case classType: ClassType =>
          val typeScope = statementScope.getScopeForType(classType.name.getQualifiedName)
          if (typeScope.gatherFields(f.name)._2.contains(Modifier.STATIC)) {
            emitStaticFieldAccessExpressionAddress(f)
          } else {
            emitNonStaticFieldAccessExpressionAddress(f)
          }
        case _ =>
          throw CodeGenerationException("should not throw")
      }
    }
    ...
    expression.variableLocation match {
      ...
      case VariableLocation.LocalField =>
        ...
        if (isStatic) {
          ...
          val staticFieldLabel = createStaticFieldLabel(classScope.cName.toString, expression.name.getFirstSimpleName.toString)
          if (expression.accessName.names.isEmpty) {
            emit(s"mov ${Reg.eax}, $staticFieldLabel", "read static field address")
          } else {
            emit(s"mov ${Reg.eax}, $staticFieldLabel", "read static field")
          }
        } else {
  ...
  def emitStringConcatenation(exp1: Expression, exp2: Expression): Unit ={
    emit("", "Begin of StringConcatenation")
    ...
    /* getString assumes the result of exp is already in Reg.eax */
    def getString(exp: Expression): Unit ={
      // we can't emitExpression here again. What if exp has side effects?
      // emitExpression(exp)
      emit("", s"Begin of getString $exp")
      ...
      exp.astType.get match{
        case PrimitiveType(PrimitiveType.INT) => emit(s"call ${createStaticMethodLabel(javaLangString.toString, "valueOf", Seq(PrimitiveType(PrimitiveType.INT)))}")
        case PrimitiveType(PrimitiveType.CHAR) => emit(s"call ${createStaticMethodLabel(javaLangString.toString, "valueOf", Seq(PrimitiveType(PrimitiveType.CHAR)))}")
        case PrimitiveType(PrimitiveType.SHORT) => emit(s"call ${createStaticMethodLabel(javaLangString.toString, "valueOf", Seq(PrimitiveType(PrimitiveType.SHORT)))}")
        case PrimitiveType(PrimitiveType.BYTE) => emit(s"call ${createStaticMethodLabel(javaLangString.toString, "valueOf", Seq(PrimitiveType(PrimitiveType.BYTE)))}")
        case PrimitiveType(PrimitiveType.BOOLEAN) => emit(s"call ${createStaticMethodLabel(javaLangString.toString, "valueOf", Seq(PrimitiveType(PrimitiveType.BOOLEAN)))}")
          // String s = null
        case ClassType(x) if x == javaLangString => emit(s"call ${createStaticMethodLabel(javaLangString.toString, "valueOf", Seq(ClassType(javaLangString)))}")
        // I think Other class and Array Type are Objects and will use the Object's valueOf method
        case ClassType(_) => emit(s"call ${createStaticMethodLabel(javaLangString.toString, "valueOf", Seq(ClassType(javaLangObject)))}")
        case ArrayType(_) => emit(s"call ${createStaticMethodLabel(javaLangString.toString, "valueOf", Seq(ClassType(javaLangObject)))}")
        case NullType() => emit(s"call ${createStaticMethodLabel(javaLangString.toString, "valueOf", Seq(ClassType(javaLangObject)))}")
        case _ =>
      }

      // if valueOf returns null, that means object.toString() returns null, so manually create "null" here
      val endLabel = createLabel()
      emit(s"cmp ${Reg.eax}, 0")
      emit(s"jne $endLabel")
      emitStringArrayCreationExpression("null")
      emitLabel(endLabel)

      emit(s"add ${Reg.esp}, 8", "pop all of the arguments and this")
      emit("", s"End of getString $exp")
    }
    emit(s"push ${Reg.ebx}", "store the second exp value")
    // Resolve the first expression into a string
    // when we first call this function, eax still holds the result of exp1
    getString(exp1)
    ...
    emit(s"add ${Reg.esp}, 8", "pop all of the arguments and this")
    emit("", "End of StringConcatenation")
  }
      ...
      case _ =>
        ...
        expression.op match {
          case BinaryOperator.PLUS =>
            (expression.exp1.astType.get, expression.exp2.astType.get) match {
              ...
              case (_, ClassType(x)) if x == javaLangString =>
                emitStringConcatenation(expression.exp1, expression.exp2)
              case _ => emit(s"add ${Reg.eax}, ${Reg.ebx}")
            }
          case BinaryOperator.MINUS =>
            emit(s"sub ${Reg.eax}, ${Reg.ebx}", "operator-")
          ...
          case BinaryOperator.LT =>
            emit(s"cmp ${Reg.eax}, ${Reg.ebx}")
            emit(s"setle ${Reg.al}", "operator<=")
            emit(s"movzx ${Reg.eax}, ${Reg.al}")
          case BinaryOperator.GT =>
            emit(s"cmp ${Reg.eax}, ${Reg.ebx}")
            emit(s"setge ${Reg.al}", "operator>=")
            emit(s"movzx ${Reg.eax}, ${Reg.al}")
          case BinaryOperator.GREATER =>
            emit(s"cmp ${Reg.eax}, ${Reg.ebx}")
            emit(s"setg ${Reg.al}", "operator>")
            emit(s"movzx ${Reg.eax}, ${Reg.al}")
          case BinaryOperator.LESS =>
            emit(s"cmp ${Reg.eax}, ${Reg.ebx}")
            emit(s"setl ${Reg.al}", "operator<")
            emit(s"movzx ${Reg.eax}, ${Reg.al}")
  ...
  def emitLocalVariableDeclarationStatement(localVariableDeclarationStatement: LocalVariableDeclarationStatement): Unit = {
    emit("", s"Begin of LocalVariableDeclarationStatement ${localVariableDeclarationStatement.name}")
    ...
    emit("", s"End of LocalVariableDeclarationStatement ${localVariableDeclarationStatement.name}")
  }
  ...
  def prologue(parameterSize: Int, isStatic: Boolean, isNativeWrite: Boolean = false): Unit = {
    ...
    emit("", "start copy saved paramters")
    for (i <- 0 until parameterSize) {
      ...
      if (isNativeWrite) {
        // native write outputs the lower-order byte of register to stdout
        // so we directly convert it here to Reg.bl
        emit(s"mov ${Reg.eax}, DWORD ${Reg.ebx}")
        emit(s"mov BYTE ${Reg.bl}, ${Reg.al}")
      }
      emit(s"mov [${Reg.esp} - ${4 * i} - 4], ${Reg.ebx}")
    }
    emit("", "finished copy saved paramters")
  ...
  def  emitStaticFields(cpUnits: Seq[CompilationUnit]): Seq[StaticFieldLabel] ={
          ...
          if(fieldDecl.modifiers.contains(Modifier.STATIC)) {
            ...
            fieldDecl.initializer match {
              case Some(expression: Expression) =>
                ...
                emit(s"mov ${Reg.ebx}, $currentFieldLabel")
                emit(s"mov [${Reg.ebx}], ${Reg.eax}")
              case None =>
                val jumpLabel = createLabel()
                emit(s"mov ${Reg.ebx}, $currentFieldLabel")
...
//            emit(s"dd ${createClassMethodLabel(interface._2.cName.toString, method._1._1.toString, method._1._2)}")
    ...
    def createSubTypeTable(curClass: QualifiedName, relationMap: mutable.LinkedHashMap[QualifiedName, Boolean]): Unit ={
      // Generate subtype table label
      emitLabel(createSubTypeTableLabel(curClass.toString))
      // First index store the curClass Index in the subtype table
      val curClassIndex = relationMap.keySet.toSeq.indexOf(curClass)
      emit(s"dd $curClassIndex")
    ...
    def createVTable(typeScope: TypeScope ): Unit = {

      val curClassEnvironment = typeScope
      ...
      createSubTypeTable(curClassEnvironment.cName, curClassEnvironment.getSubClassRelation)
    ...
    // Consider the case Object o = new Serializable[2];
    // In this case we will need an interface's Vtable
    def createInterfaceVtable(objectScope: TypeScope): Unit = {
      emitLabel(createVtableLabel(objectScope.cName.toString))
      // Array's VTable does not have SIT nor Subtype Table, therefore, they are set to 0
      emit("dd 0")
      emit("dd 0")
    }
    ...
    def recur(curEnvironment : Environment, ast: Ast): Unit ={
      ast match {
        ...
        case classDecl: ClassDecl =>
          ...
          emit("")
          emit("")
          createVTable(classDecl.environment.get.asInstanceOf[TypeScope])
          classDecl.children.foreach(x => recur(classDecl.environment.get.asInstanceOf[TypeScope], x))
        case interfaceDecl: InterfaceDecl =>
          createInterfaceVtable(interfaceDecl.environment.get.asInstanceOf[TypeScope])
  ...
  def codeGeneration(cpUnits: Seq[CompilationUnit]): Unit = {
    ...
    emitNativeWrite()
    emit("_start:")
    val staticFieldLabels = emitStaticFields(cpUnits)
    emitStartMethod(cpUnits.head)
    emit("call __debexit")
    emit("section .data")
    emitStaticFieldsData(staticFieldLabels)
    emit("section .text")
  }
