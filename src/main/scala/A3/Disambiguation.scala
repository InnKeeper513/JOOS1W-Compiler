...
import A2.Asts.VariableLocation
import A2.Asts._
          ...
          parenthesesExpression.exp match {
            case variableExpression: VariableExpression =>
              try {
                ...
                val statementScope = parenthesesExpression.environment.get.asInstanceOf[StatementScope]
                if (variableExpression.name.getQualifiedName.names.size == 1) {
                  val variable = statementScope.resolveVariable(variableExpression.name.getFirstSimpleName)
                  if (variable.isEmpty) {
    ...
    def typeOf(exp: Expression, allowInstance: Boolean = true): Type = {
      ...
      exp match {
        ...
        case notExpression: NotExpression if typeOf(notExpression.exp, allowInstance) == PrimitiveType(PrimitiveType.BOOLEAN) =>
          notExpression.astType = Some(PrimitiveType(PrimitiveType.BOOLEAN))
        ...
        case binaryExpression: BinaryExpression if
          ...
          (typeOf(binaryExpression.exp2, allowInstance), typeOf(binaryExpression.exp1, allowInstance)) match {
            case (r1: ArrayType, r2: ArrayType) =>
              (r1.aType, r2.aType) match {
                case (r3: PrimitiveType, r4: PrimitiveType) =>
                case (r3: ReferenceType, r4: ReferenceType) =>
                  if (!checkTypeCastable(r3, r4)) throw TypeCheckingException("..?>?>./")
                case _ =>
          ...
          PrimitiveType(PrimitiveType.BOOLEAN)
        case binaryExpression: BinaryExpression if
            Seq(BinaryOperator.LAND, BinaryOperator.LOR, BinaryOperator.EAND, BinaryOperator.EOR).contains(binaryExpression.op) &&
            typeOf(binaryExpression.exp2, allowInstance) == PrimitiveType(PrimitiveType.BOOLEAN) && typeOf(binaryExpression.exp1, allowInstance) == PrimitiveType(PrimitiveType.BOOLEAN) =>
          binaryExpression.astType = Some(PrimitiveType(PrimitiveType.BOOLEAN))
          PrimitiveType(PrimitiveType.BOOLEAN)
        ...
        case methodInvocationExpression: MethodInvocationExpression =>
          ...
          if (eType.isInstanceOf[ClassType] || eType.isInstanceOf[InterfaceType]) {
            var name: QualifiedName = null
            if (eType.isInstanceOf[ClassType]) {
              name = eType.asInstanceOf[ClassType].name.getQualifiedName
            } else {
              name = eType.asInstanceOf[InterfaceType].name.getQualifiedName
            }
            val classScope = rootEnvironment.getScopeForType(name)
            ...
            throw TypeCheckingException(methodInvocationExpression.toString)
          }
        case arrayAccessExpression: ArrayAccessExpression =>
