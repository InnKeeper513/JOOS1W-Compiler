...
import A2.Environments.Environment
import scala.util.control.Breaks._

object Asts {

  case class AstException(message: String = "") extends Throwable {
  ...
  *****/
  ...
  case class IntegerLiteral(lexeme: String) extends Literal(lexeme) {
    ...
    def getLexeme: Int = lexeme.toInt
  }
  ...
  case class CharLiteral(lexeme: String) extends Literal(lexeme) {
    ...
    def getLexeme: Char = {
      convertLiteralChar(lexeme.slice(1, lexeme.length - 1))._1.toChar
    }
  }
  ...
  case class StringLiteral(lexeme: String) extends Literal(lexeme) {
    ...
    def getLexeme: String = {
      var s = lexeme.slice(1, lexeme.length - 1)
      var i = 0
      while (i <= s.length) {
        var j = Math.min(s.length - i, 4)
        breakable {
          while (i + j <= s.length && j > 1) {
            val substitute = convertLiteralChar(s.slice(i, i + j))
            if (substitute._2 > 0) {
              val first = s.slice(0, i)
              val last = s.slice(i + substitute._2, s.length)
              s = first + substitute._1.toChar + last
              break
            }
            j = j - 1
          }
  ...
  case class BooleanLiteral(lexeme: String) extends Literal(lexeme) {
    ...
    def getLexeme: Boolean = lexeme.toBoolean
  }
  ...
  def convertLiteralChar(lexeme: String): (Int, Int) = lexeme match {
    case "\\n" => ('\n', 2)
    case "\\r" => ('\r', 2)
    case "\\t" => ('\t', 2)
    case "\\b" => ('\b', 2)
    ...
    case "\\\\" => ('\\', 2)
    case "\\'" => ('\'', 2)
    case s if s.length == 1 => (s.charAt(0), 1)
    case s if s.length > 1 && s.length <= 4 && s(0) == '\\' =>
      try {
        def checkCharRange(c: Char, lowerBound: Char, upperBound: Char): Boolean = c >= lowerBound && c <= upperBound
        val octalString = s.slice(1, s.length)
        if (octalString.length == 3) {
          if (!checkCharRange(octalString(0), '0', '3')) return (0, 0)
          if (!checkCharRange(octalString(1), '0', '7')) return (0, 0)
          if (!checkCharRange(octalString(2), '0', '7')) return (0, 0)
        } else {
          for (i <- 0 until octalString.length) {
            if (!checkCharRange(octalString(i), '0', '7')) return (0, 0)
          }
        }
        (Integer.parseInt(octalString, 8), s.length)
      } catch {
  ...
  def getExpressionAst(expTree: Tree): Expression = {
    expTree.lhs.kind match {
      ...
      case "UnaryExpression" if expTree.size == 2 =>
        ...
        childExp match {
          case LiteralExpression(IntegerLiteral(lexeme)) =>
            if (lexeme.startsWith("-")) LiteralExpression(IntegerLiteral(lexeme.slice(1, lexeme.length)))
            else LiteralExpression(IntegerLiteral("-" + lexeme))
        ...
        if (expTree.production == "CastExpression LEFT_PAREN PrimitiveType Dims RIGHT_PAREN UnaryExpression") {
          ...
          val variableExpression: VariableExpression = {
            try {
              ...
              getExpressionAst(expTree.children(1)) match {
                case v: VariableExpression => v
                case _ => throw AstException("Cannot cast " + getExpressionAst(expTree.children(1)) + " into VariableExpression")
              }
            } catch {
